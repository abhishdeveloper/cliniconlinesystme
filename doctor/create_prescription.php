...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $prakruti = !empty($_POST['prakruti']) ? $_POST['prakruti'] : null;
    $vikruti = !empty($_POST['vikruti']) ? $_POST['vikruti'] : null;
    $agni = !empty($_POST['agni']) ? $_POST['agni'] : null;
    ...
    if (empty($diagnosis)) {
        ...
        try {
            ...
            if ($check->fetch()) {
                // Update existing
                $stmt = $pdo->prepare("UPDATE prescriptions SET diagnosis = ?, medicines_json = ?, notes = ?, prakruti = ?, vikruti = ?, agni = ? WHERE appointment_id = ?");
                $stmt->execute([$diagnosis, $medicines_json, $notes, $prakruti, $vikruti, $agni, $appointment_id]);
                setFlashMessage('success', "Prescription updated successfully!", 'success');
            } else {
                // Create new
                $stmt = $pdo->prepare("INSERT INTO prescriptions (appointment_id, patient_id, doctor_id, diagnosis, medicines_json, notes, prakruti, vikruti, agni) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$appointment_id, $appointment['patient_id'], $_SESSION['user_id'], $diagnosis, $medicines_json, $notes, $prakruti, $vikruti, $agni]);
    ...
    <div class="card shadow">
        ...
        <div class="card-body">
            <form method="POST" id="prescriptionForm">
                ...
                <h5 class="mb-3 text-success border-bottom pb-2">Ayurvedic Assessment</h5>
                <div class="row mb-4">
                    <div class="col-md-4">
                        <label class="form-label fw-bold">Prakruti (Constitution)</label>
                        <select name="prakruti" class="form-select">
                            <option value="">Select Prakruti</option>
                            ...
                            <option value="Tridosha">Tridosha (Vata-Pitta-Kapha)</option>
                        </select>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label fw-bold">Vikruti (Imbalance)</label>
                        <select name="vikruti" class="form-select">
                    ...
                    <div class="col-md-4">
                        <label class="form-label fw-bold">Agni (Digestive Fire)</label>
                        <select name="agni" class="form-select">
                            ...
                            <option value="Tikshna Agni">Tikshna Agni (Sharp - Pitta)</option>
                            <option value="Manda Agni">Manda Agni (Dull - Kapha)</option>
                        </select>
                    </div>
                </div>
