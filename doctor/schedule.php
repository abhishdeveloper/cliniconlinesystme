...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'generate_slots') {
    ...
    try {
        ...
        $existing_slots_stmt->execute([$_SESSION['user_id'], $start_of_day]);
        $existing_slots = array_flip($existing_slots_stmt->fetchAll(PDO::FETCH_COLUMN));

        $pdo->beginTransaction(); // Wrap bulk inserts in a transaction to prevent disk sync overhead
            ...
            foreach ($templates as $template) {
                if ($template['day_of_week'] === $day_name) {
                    ...
                    while ($start < $end) {
                        ...
                        // Check if slot exists in O(1) time
                        if (!isset($existing_slots[$slot_datetime])) {
                            $insert->execute([$_SESSION['user_id'], $slot_datetime]);
                            $existing_slots[$slot_datetime] = true; // Add to in-memory map to prevent duplicates during generation
                            $generated_count++;
                        }
