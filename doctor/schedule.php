...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'generate_slots') {
    ...
    try {
        ...
        // Fetch existing slots to prevent N+1 queries
        $slots_stmt = $pdo->prepare("SELECT slot_datetime FROM appointment_slots WHERE doctor_id = ? AND slot_datetime >= ? AND slot_datetime <= ?");
        $slots_stmt->execute([$_SESSION['user_id'], $today->format('Y-m-d 00:00:00'), $end_date->format('Y-m-d 23:59:59')]);

        // Create an O(1) in-memory hash map for fast existence checking
        $existing_slots = array_flip($slots_stmt->fetchAll(PDO::FETCH_COLUMN));
            ...
            foreach ($templates as $template) {
                if ($template['day_of_week'] === $day_name) {
                    ...
                    while ($start < $end) {
                        ...
                        // Check if slot exists in memory
                        if (!isset($existing_slots[$slot_datetime])) {
                            $insert->execute([$_SESSION['user_id'], $slot_datetime]);
                            $existing_slots[$slot_datetime] = true; // Track newly inserted to avoid duplication
                            $generated_count++;
                        }
