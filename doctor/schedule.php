...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'generate_slots') {
    ...
    try {
        ...
        $today = new DateTime();
        $start_period = $today->format('Y-m-d 00:00:00');
        $end_period = (clone $today)->modify("+$days_ahead days")->format('Y-m-d 23:59:59');

        // Fetch existing slots to avoid N+1 queries
        $existing_stmt = $pdo->prepare("SELECT slot_datetime FROM appointment_slots WHERE doctor_id = ? AND slot_datetime BETWEEN ? AND ?");
        $existing_stmt->execute([$_SESSION['user_id'], $start_period, $end_period]);
