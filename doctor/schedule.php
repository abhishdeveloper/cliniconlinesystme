...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'generate_slots') {
    ...
    try {
        ...
        // ⚡ Bolt Optimization: Fetch all existing slots in one query to prevent N+1 queries in loop
        ...
        $existing_slots_stmt->execute([$_SESSION['user_id'], $today->format('Y-m-d H:i:s'), $end_date->format('Y-m-d 23:59:59')]);

        // Create an O(1) lookup hash map of existing slots
        $existing_slots = array_flip($existing_slots_stmt->fetchAll(PDO::FETCH_COLUMN));

        // ⚡ Bolt Optimization: Wrap batch inserts in a single transaction to reduce disk I/O
            ...
            foreach ($templates as $template) {
                if ($template['day_of_week'] === $day_name) {
                    ...
                    while ($start < $end) {
                        ...
                        // O(1) check using hash map
                        if (!isset($existing_slots[$slot_datetime])) {
                            $insert->execute([$_SESSION['user_id'], $slot_datetime]);
                            // Update local map to prevent duplicates within the same run if templates overlap
