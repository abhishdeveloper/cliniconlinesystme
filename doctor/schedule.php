...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'generate_slots') {
    ...
    try {
        ...
        // ⚡ Bolt: Fetch existing slots for O(1) hash map lookup to prevent N+1 queries
        $start_date_str = $today->format('Y-m-d 00:00:00');
        $existing_slots_stmt = $pdo->prepare("SELECT slot_datetime FROM appointment_slots WHERE doctor_id = ? AND slot_datetime >= ?");
        $existing_slots_stmt->execute([$_SESSION['user_id'], $start_date_str]);
        $existing_slots = array_flip($existing_slots_stmt->fetchAll(PDO::FETCH_COLUMN));

        // ⚡ Bolt: Wrap inserts in a transaction to eliminate massive I/O overhead for bulk inserts
                ...
                if ($template['day_of_week'] === $day_name) {
                    ...
                    while ($start < $end) {
                        ...
                        if (!isset($existing_slots[$slot_datetime])) {
                            ...
                            $existing_slots[$slot_datetime] = true; // Add to hash map to prevent duplicate generation within the same run
                            $generated_count++;
                        }
