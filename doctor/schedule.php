...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'generate_slots') {
    ...
    try {
        ...
        // Fetch existing slots in one query for O(1) lookups
        ...
        $existing_slots_raw = $existing_stmt->fetchAll(PDO::FETCH_COLUMN);

        // Convert to hash map for O(1) checks
        $existing_slots = [];
        if ($existing_slots_raw) {
            $existing_slots = array_flip($existing_slots_raw);
        }
            ...
            foreach ($templates as $template) {
                if ($template['day_of_week'] === $day_name) {
                    ...
                    while ($start < $end) {
                        ...
                        // O(1) lookup instead of N+1 SELECT query
                        if (!isset($existing_slots[$slot_datetime])) {
                            $insert->execute([$_SESSION['user_id'], $slot_datetime]);
                            $existing_slots[$slot_datetime] = true; // Prevent duplicates in same run
                            $generated_count++;
                        }
