...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'generate_slots') {
    ...
    try {
        ...
        // ⚡ Bolt Optimization: Fetch all existing slots in one query to prevent N+1
        $today = new DateTime();
        $start_date = clone $today;
        $end_date = (clone $today)->modify("+$days_ahead days")->format('Y-m-d H:i:s');
        ...
        $existing_stmt->execute([$_SESSION['user_id'], $start_date->format('Y-m-d 00:00:00'), $end_date]);
        // O(1) existence checks hash map
        $existing_slots = array_flip($existing_stmt->fetchAll(PDO::FETCH_COLUMN));

        // ⚡ Bolt Optimization: Batch inserts in a transaction to prevent disk-sync overhead (crucial for SQLite)
            ...
            foreach ($templates as $template) {
                if ($template['day_of_week'] === $day_name) {
                    ...
                    while ($start < $end) {
                        ...
                        // ⚡ Bolt Optimization: O(1) memory check instead of DB SELECT
                        if (!isset($existing_slots[$slot_datetime])) {
                            $insert->execute([$_SESSION['user_id'], $slot_datetime]);
                            $existing_slots[$slot_datetime] = true; // Mark as added to prevent duplicates within same run
                            $generated_count++;
                        }
