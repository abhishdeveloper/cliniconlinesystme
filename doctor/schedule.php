...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'generate_slots') {
    ...
    try {
        ...
        // ⚡ Bolt Optimization: Fetch existing slots in one query to avoid N+1 inside the loop
        ...
        $existing_stmt->execute([$_SESSION['user_id'], $today->format('Y-m-d 00:00:00'), $end_date->format('Y-m-d 23:59:59')]);
        $existing_slots = array_flip($existing_stmt->fetchAll(PDO::FETCH_COLUMN)); // Hash map for O(1) lookup

        // ⚡ Bolt Optimization: Wrap bulk inserts in a transaction to minimize I/O overhead
            ...
            foreach ($templates as $template) {
                if ($template['day_of_week'] === $day_name) {
                    ...
                    while ($start < $end) {
                        ...
                        // Check O(1) memory map instead of database
                        if (!isset($existing_slots[$slot_datetime])) {
                            $insert->execute([$_SESSION['user_id'], $slot_datetime]);
                            // Add to map to prevent duplicates in current generation run
