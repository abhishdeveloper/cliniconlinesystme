...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'generate_slots') {
    ...
    try {
        ...
        $today = new DateTime();
        $end_date_str = (clone $today)->modify("+$days_ahead days")->format('Y-m-d 23:59:59');

        // Fetch existing slots into an in-memory hash map for O(1) lookups
        $stmt_existing = $pdo->prepare("SELECT slot_datetime FROM appointment_slots WHERE doctor_id = ? AND slot_datetime >= ? AND slot_datetime <= ?");
        $stmt_existing->execute([$_SESSION['user_id'], $today->format('Y-m-d 00:00:00'), $end_date_str]);
        $existing_slots = array_flip($stmt_existing->fetchAll(PDO::FETCH_COLUMN));

        $pdo->beginTransaction();

        // Prepare insert statement outside the loop
            ...
            foreach ($templates as $template) {
                if ($template['day_of_week'] === $day_name) {
                    ...
                    while ($start < $end) {
                        ...
                        // Check if slot exists using the hash map
