...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'generate_slots') {
    ...
    try {
        ...
        // Fetch existing slots into a hash map for O(1) lookups to prevent N+1 queries
        $start_of_generation = $today->format('Y-m-d 00:00:00');
        $existing_stmt = $pdo->prepare("SELECT slot_datetime FROM appointment_slots WHERE doctor_id = ? AND slot_datetime >= ?");
        $existing_stmt->execute([$_SESSION['user_id'], $start_of_generation]);
                ...
                if ($template['day_of_week'] === $day_name) {
                    ...
                    while ($start < $end) {
                        ...
                        if (!isset($existing_slots[$slot_datetime])) {
                            ...
                            $existing_slots[$slot_datetime] = true; // Add to local map to prevent duplicates in current run
                            $generated_count++;
                        }
