...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'generate_slots') {
    ...
    try {
        ...
        // ⚡ Bolt: Fetch existing slots in a single query (O(1) hash map lookup) to avoid N+1 query overhead
        ...
        $stmt_existing->execute([$_SESSION['user_id'], $today->format('Y-m-d 00:00:00'), $end_date->format('Y-m-d 23:59:59')]);
        $existing_slots = array_flip($stmt_existing->fetchAll(PDO::FETCH_COLUMN));

        // ⚡ Bolt: Wrap inserts in a single transaction to significantly reduce SQLite disk I/O overhead
            ...
            foreach ($templates as $template) {
                if ($template['day_of_week'] === $day_name) {
                    ...
                    while ($start < $end) {
                        ...
                        // Check if slot exists using the array map
                        if (!isset($existing_slots[$slot_datetime])) {
                            $insert->execute([$_SESSION['user_id'], $slot_datetime]);
                            $existing_slots[$slot_datetime] = true; // Prevent duplicate slots from overlapping templates
                            $generated_count++;
                        }
