...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'generate_slots') {
    ...
    try {
        ...
        // Fetch all existing slots for this doctor to prevent N+1 queries
        $existing_stmt = $pdo->prepare("SELECT slot_datetime FROM appointment_slots WHERE doctor_id = ? AND slot_datetime >= ?");
        $existing_stmt->execute([$_SESSION['user_id'], $today->format('Y-m-d 00:00:00')]);
        $existing_slots = array_flip($existing_stmt->fetchAll(PDO::FETCH_COLUMN));

        $pdo->beginTransaction(); // Wrap inserts in a transaction to prevent disk I/O bottlenecks
            ...
            foreach ($templates as $template) {
                if ($template['day_of_week'] === $day_name) {
                    ...
                    while ($start < $end) {
                        ...
                        // O(1) in-memory check instead of N+1 database queries
                        if (!isset($existing_slots[$slot_datetime])) {
                            $insert->execute([$_SESSION['user_id'], $slot_datetime]);
                            $existing_slots[$slot_datetime] = true; // Add to local map to prevent overlaps within same batch
                            $generated_count++;
                        }
