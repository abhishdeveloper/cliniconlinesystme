...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'generate_slots') {
    ...
    try {
        ...
        // Fetch all existing slots for this doctor from today onwards
        $stmt_existing = $pdo->prepare("SELECT slot_datetime FROM appointment_slots WHERE doctor_id = ? AND slot_datetime >= ?");
        $stmt_existing->execute([$_SESSION['user_id'], $today_start]);
        $existing_slots_arr = $stmt_existing->fetchAll(PDO::FETCH_COLUMN);

        // Convert to an O(1) hash map for quick lookup
        $existing_slots = array_flip($existing_slots_arr);
            ...
            foreach ($templates as $template) {
                if ($template['day_of_week'] === $day_name) {
                    ...
                    while ($start < $end) {
                        ...
                        // Check O(1) hash map instead of N+1 database queries
                        if (!isset($existing_slots[$slot_datetime])) {
                            $insert->execute([$_SESSION['user_id'], $slot_datetime]);
                            $generated_count++;
                            // Add newly created slot to the hash map to prevent duplicates during the same generation run
