...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'generate_slots') {
    ...
    try {
        ...
        // ⚡ Bolt: Fetch all existing slots for the next 30 days into an in-memory hash map to prevent N+1 queries
        $end_date_str = (clone $today)->modify("+$days_ahead days")->format('Y-m-d H:i:s');
        $start_date_str = $today->format('Y-m-d 00:00:00'); // Use start of day

        $existing_stmt = $pdo->prepare("SELECT slot_datetime FROM appointment_slots WHERE doctor_id = ? AND slot_datetime >= ? AND slot_datetime <= ?");
        $existing_stmt->execute([$_SESSION['user_id'], $start_date_str, $end_date_str]);
        $existing_slots = array_flip($existing_stmt->fetchAll(PDO::FETCH_COLUMN));

        // ⚡ Bolt: Prepare INSERT statement once outside the loop
        $insert = $pdo->prepare("INSERT INTO appointment_slots (doctor_id, slot_datetime) VALUES (?, ?)");

        // ⚡ Bolt: Wrap generation in a transaction to minimize disk I/O overhead
            ...
            foreach ($templates as $template) {
                if ($template['day_of_week'] === $day_name) {
                    ...
                    while ($start < $end) {
                        ...
                        // ⚡ Bolt: O(1) existence check instead of querying DB
                        if (!isset($existing_slots[$slot_datetime])) {
                            $insert->execute([$_SESSION['user_id'], $slot_datetime]);
                            $existing_slots[$slot_datetime] = true; // Add to map to prevent duplicates
                            $generated_count++;
                        }
