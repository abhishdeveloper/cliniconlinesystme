...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'generate_slots') {
    ...
    try {
        ...
        // ⚡ Bolt: Fetch all existing slots for O(1) hash map lookup
        // Prevents N+1 queries during slot generation
        ...
        // ⚡ Bolt: Begin transaction to prevent severe disk sync I/O overhead on mass inserts
            ...
            foreach ($templates as $template) {
                if ($template['day_of_week'] === $day_name) {
                    ...
                    while ($start < $end) {
                        ...
                        // ⚡ Bolt: Use O(1) hash map lookup instead of N+1 database SELECT queries
                        if (!isset($existing_slots[$slot_datetime])) {
                            $insert_stmt->execute([$_SESSION['user_id'], $slot_datetime]);
                            $existing_slots[$slot_datetime] = true; // Mark as existing to avoid duplicate inserts on same pass
                            $generated_count++;
                        }
