...
if (isset($_GET['action']) && $_GET['action'] === 'get_slots') {
    ...
    try {
        ...
        echo '<option value="">Error loading slots. Please try again.</option>';
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'book_appointment') {
    ...
    if (empty($doctor_id) || empty($slot_id)) {
        ...
        try {
            ...
            setFlashMessage('danger', "An unexpected error occurred while booking the appointment.", 'danger');
        }
    }
}
                        ...
                        <table class="table table-hover align-middle">
                            ...
                            <tbody>
                                ...
                                if (count($appointments) > 0) {
                                    ...
                                    error_log("Appointment Load Error: " . $e->getMessage());
                                    echo "<tr><td colspan='4' class='text-danger'>An unexpected error occurred while loading appointments.</td></tr>";
                                }
                                ?>
                            </tbody>
                        </table>
